...
LEFT JOIN `item numbers, descrip, page` d ON d.itemId = p.ProductId";
		...
		public void GetParentIds()
		{
			try
			{
				...
				Debug.Assert(parents.categoryName == CategoryName && parents.categoryId == CategoryId);
				Debug.Assert(parents.subCategoryName == SubCategoryName && parents.subCategoryId == SubCategoryId);
				Debug.Assert(parents.productName == ProductName && parents.productId == ProductId);
			}
			catch (Exception ex)
			{
